...
//              + "01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
    ...
    private static int[][] createGrid(String gridText)
    {
        ...
        final List<List<String>> values = lines.stream()
                                               .map(line -> asList(line.trim().split(" ")))
                                               .collect(toList());
...
//        System.out.println(Arrays.deepToString(GRID));
        ...
        for(int i = 0; i < GRID.length; i++) // each row
        {
            for(int j = 0; j < GRID[0].length; j++) // each column
            {
                ...
                int right = 1;
                int diagonal = 1;
                if((i + n) < GRID.length) // we do not want to go out of the grid from bottom
                {
                    for(int k = 0; k < n; k++)
                    {
                        System.out.print("," + k);
                ...
                if((j + n) < GRID[0].length) // we do not want to go out of the grid from right
                {
                    for(int k = 0; k < n; k++)
                    {
                        System.out.print("," + k);
                ...
                if((j + n) < GRID[0].length && (i + n) < GRID.length) // we do not want to go out of the grid from right
                {
                ...
                maxProduct = Math.max(maxProduct, Math.max(right, Math.max(down, diagonal)));
                maxI = i;
                maxJ = j;
                System.out.println("max product:" + maxProduct + ", right:" + right + ", down:" + down + ", diagonal:" + diagonal);
            }
        }
        return maxProduct;
    }
